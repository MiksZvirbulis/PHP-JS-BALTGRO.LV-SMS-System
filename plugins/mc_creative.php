...
*/
$c[$p]['db']['table'] = "baltsms_mc_creative";
...
*/
$c[$p]['ingame']['message'] = "<NICKNAME> just purchased creative gamemode from our BaltSMS Shop!";

/*
    Frakcijas peaceful iedošanas komanda. Pēc noklusējuma, pievienota Essentials komanda
*/
$c[$p]['commands']['giveCreative'][] = "pex user <NICKNAME> add essentials.gamemode";
//$c[$p]['commands']['giveCreative'][] = "pex user <NICKNAME> prefix &6[Gamemode]&f"; // atkomentējam, ja vēlamies spēlētāja nikam priekš ievietot tekstu

/*
    Frakcijas peaceful noņemšanas komanda. Pēc noklusējuma, pievienota Essentials komanda
*/
$c[$p]['commands']['removeCreative'][] = "pex user <NICKNAME> remove essentials.gamemode";
//$c[$p]['commands']['removeCreative'][] = "pex user <NICKNAME> prefix &f"; // atkomentējam, lai, pēc termiņa beigām noņemas spēlētāja nika teksts


// CENU KODS => DIENU SKAITS
$c[$p]['prices'] = array(
    "test" => array(
    	740 => 14
    ),
...
$c['lang'][$p]['lv'] = array(
    "instructions" => "Lai iegādātos creative uz <LENGTH> dienām par <PRICE> EUR, sūti kodu <b><KEYWORD><CODE></b> uz <b><NUMBER></b>, lai saņemtu atslēgas kodu!",
	# Kļūdas
    ...
    "creative_purchased" => "Creative gamemode veiksmīgi iegādāts. Lai jauka spēlēšana!",
	# Forma
    "form_price" => "Cena",
    ...
    "table_no_buyers" => "Neviens vēl nav iegādājies creative gamemode. Varbūt vēlies būt pirmais?"
);

$c['lang'][$p]['en'] = array(
	"instructions" => "To purchase creative gamemode for <LENGTH> days for <PRICE> EUR, send the following code: <b><KEYWORD><CODE></b> to <b><NUMBER></b> to receive an unclock code!",
	# Kļūdas
	...
	"creative_purchased" => "Creative gamemode was purchased successfully. Have fun!",
	# Forma
	"form_price" => "Price",
    ...
    "table_no_buyers" => "No one has bought creative gamemode yet. Would you like to be the first?"
);
/*
-----------------------------------------------------
...
if(isset($_POST['code'])):
	...
	if(count($errors) > 0){
		...
		if($baltsms->getResponse() === true){
			...
			foreach($c[$p]['commands']['giveCreative'] as $command){
				$giveCreative = str_replace(
					array("<NICKNAME>"),
					array($_POST['nickname']),
					$command
				);
				$mc['rcon'][$_POST['server']]->send_command($giveCreative);
			}
			...
			echo baltsms::alert($lang[$p]['creative_purchased'], "success");
			?>
...
foreach($purchases as $purchase){
	if($purchase['expires'] <= time()){
		if($mc['rcon'][$purchase['server']]->connect() != false){
			foreach($c[$p]['commands']['removeCreative'] as $command){
				$removeCreative = str_replace(
					array("<NICKNAME>"),
					array($_POST['nickname']),
					$command
				);
				$mc['rcon'][$purchase['server']]->send_command($removeCreative);
			}
